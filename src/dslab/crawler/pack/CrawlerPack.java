...
public class CrawlerPack {
	 ...
	 *
	 */
	public CrawlerPack addCookie(String domain, String name, String value, String path, Date expires, boolean secure) {
		if (null == name) {
			log.warn("Cookie name null.");
			return this;
		}
	...
	/**
	 * Return a Cookie array and auto importing domain and path when domain was
	 * empty.
	 *
	 * @param uri
	 *            required Apache Common VFS supported file systems and response
	 *            JSON format content.
	 * @return Cookie[]
	 */
	...
	public String getFromRemote(String uri) {
		...
		try {
			...
			if ("http".equalsIgnoreCase(uri.substring(0, 4))) {
				fileContent.getSize(); // pass a bug {@link
